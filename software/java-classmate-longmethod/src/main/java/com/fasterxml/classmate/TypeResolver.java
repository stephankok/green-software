...
public class TypeResolver implements Serializable
{
    ...
    public ResolvedType resolve(Type type, Type... typeParameters)
    {
        ...
        ResolvedType[] resolvedParams = resolveTypeParameters(typeParameters, bindings);
		return _fromClass(null, rawBase, TypeBindings.create(rawBase, resolvedParams));
    }

	private ResolvedType[] resolveTypeParameters(Type[] typeParameters, TypeBindings bindings) {
        ...
        throws IllegalArgumentException, UnsupportedOperationException
    {
        ...
        TypePlaceHolder[] placeholders = instantiatePlaceholders(paramCount);
...
//            resolvedSubtype = resolve(subtype);
        ...
        ResolvedType[] typeParams = reconstructTypeParams(subtype, paramCount, placeholders);
		return resolve(subtype, typeParams);
    }

	private TypePlaceHolder[] instantiatePlaceholders(int paramCount) {
		TypePlaceHolder[] placeholders = null;
		if (paramCount != 0) {
	...
	private ResolvedType[] reconstructTypeParams(final Class<?> subtype, int paramCount, TypePlaceHolder[] placeholders)
		...
		for (int i = 0; i < paramCount; ++i) {
			...
			if (t == null) {
				throw new IllegalArgumentException(
						"Failed to find type parameter #" + (i + 1) + "/" + paramCount + " for " + subtype.getName());
			}
			typeParams[i] = t;
		}
		return typeParams;
    ...
    private ResolvedType _constructType(ClassStack context, Class<?> rawType, TypeBindings typeBindings)
    {
        ...
        typeBindings = excludeEmptyTypeBindings(rawType, typeBindings);
	...
	private TypeBindings excludeEmptyTypeBindings(Class<?> rawType, TypeBindings typeBindings) {
    ...
    private ResolvedType _fromParamType(ClassStack context, ParameterizedType ptype, TypeBindings parentBindings)
    {
        ...
        ResolvedType[] types = getActualTypeArguments(context, ptype, parentBindings);
		// Ok: this gives us current bindings for this type:
        TypeBindings newBindings = TypeBindings.create(rawType, types);
        return _fromClass(context, rawType, newBindings);
    }

	private ResolvedType[] getActualTypeArguments(ClassStack context, ParameterizedType ptype,
